...
<div class="main-page-content view" id="tblSticky"    style="top: 0 !important;">
	...
	<input type="hidden" name="fromHandler" value="true">
		...
		<tr><td colspan='2'>
							<input type="text" name="owner" class="text-space" value="${entry.owner}" readonly="readonly"/>
							
							<%-- <c:choose>
							<c:when test="${entry.isHandler}">
							<script>
								window.onload = function() {
							    document.getElementById("isHandler").checked = true;
								}
							</script>
							</c:when>
							<c:otherwise>
							<script>
								window.onload = function() {
							    document.getElementById("isHandler").checked = false;
								}
							</script>
							</c:otherwise>
							</c:choose> --%>
							<!-- <input type="checkbox" name="ishandler" class=".w3-check" value="True" style="padding:0px 15px !important;" id="isHandler"><label for="isHandler"> Handler</label>
							 -->
